...
/*         ScoreArray single_1_in_score_index(scale);  //on stack
        single_1_in_score_index[score] = 1; //now its an array with zeros except for in index score
        
        group.group_levels.updateRank(0, &single_1_in_score_index); //TODO: how do we update number of players in level data?
                                                                    //TODO: maybe it should be done by running on the array in rank but its stupid

        all_players_by_level.updateRank(0, &single_1_in_score_index);

        int& group_num_of_players_in_level = group.group_levels.AVLGet(0);
        group_num_of_players_in_level++;

        int& all_num_of_players_in_level = all_players_by_level.AVLGet(0);
